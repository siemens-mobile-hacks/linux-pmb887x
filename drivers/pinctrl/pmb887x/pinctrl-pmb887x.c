...
#define PMB887X_PINCONF_PAD_REG (PIN_CONFIG_END + 1)
...
static int pmb887x_gpio_direction_input(struct gpio_chip *chip, unsigned pin)
{
	...
	if (pctl->pins_alt_func[pin]) {
		dev_err(pctl->dev,
			"pin %d in the alt mode, can't change direction.\n",
			pin);
		return -EINVAL;
	}
...
static int pmb887x_gpio_direction_output(struct gpio_chip *chip, unsigned pin,
					 int value)
{
	struct pmb887x_pinctrl *pctl = gpiochip_get_data(chip);
	u32 reg = GPIO_DIR_OUT | GPIO_PS_MANUAL | GPIO_ENAQ_OFF |
		  (value ? GPIO_DATA_HIGH : GPIO_DATA_LOW);
...
static int pmb887x_gpio_get_direction(struct gpio_chip *chip, unsigned int pin)
{
	struct pmb887x_pinctrl *pctl = gpiochip_get_data(chip);
	u32 reg = readl_relaxed(pctl->base + GPIO_PIN(pin));
	return ((reg & GPIO_DIR) == GPIO_DIR_IN ? GPIO_LINE_DIRECTION_IN :
...
static int pmb887x_dt_subnode_to_map(struct pinctrl_dev *pctldev,
				     struct device_node *np,
				     struct pinctrl_map **map,
				     unsigned int *reserved_maps,
				     unsigned int *num_maps)
{
	const char *propname = "pmb887x,pads";
	int rows, i, ret;
	...
	for (i = 0; i < rows; i++) {
		...
		ret = pinctrl_parse_index_with_args(np, propname, i, &spec);
		if (ret)
			return ret;
		if (spec.args_count < 2)
			return -EINVAL;

		pin_name = pin_get_name(pctldev, spec.args[0]);
		if (!pin_name)
			return -EINVAL;

		config = pinconf_to_config_packed(PMB887X_PINCONF_PAD_REG,
						  spec.args[1]);
		ret = pinctrl_utils_add_map_configs(pctldev, map, reserved_maps,
						    num_maps, pin_name, &config,
						    1,
...
static int pmb887x_dt_node_to_map(struct pinctrl_dev *pctldev,
				  ...
				  unsigned int *num_maps)
{
	...
	*num_maps = 0;
	...
	ret = pmb887x_dt_subnode_to_map(pctldev, np_config, map, &reserved_maps,
					num_maps);
	if (ret < 0)
		goto exit;

	for_each_available_child_of_node_scoped(np_config, np) {
		ret = pmb887x_dt_subnode_to_map(pctldev, np, map,
...
static const struct pinctrl_ops pmb887x_pctl_ops = {
	...
	.dt_node_to_map = pmb887x_dt_node_to_map,
...
static void pmb887x_pmx_free(struct pinctrl_dev *pctldev,
...
static const struct pmb887x_desc_mux *
pmb887x_pmx_find_mux(struct pmb887x_pinctrl *pctl, int pin, const char *name)
{
	int i;
	for (i = 0; i < PMB887X_CONFIG_NUM; i++) {
		const struct pmb887x_desc_mux *mux =
			&pctl->pins[pin].functions[i];
...
static int pmb887x_pmx_set_mux(struct pinctrl_dev *pctldev,
			       ...
			       unsigned int group_select)
{
	...
	for (i = 0; i < group->grp.npins; i++) {
		...
		function_name =
			pinmux_generic_get_function_name(pctldev, func_select);

		const struct pmb887x_desc_mux *mux = pmb887x_pmx_find_mux(
			pctl, group->grp.pins[i], function_name);
		if (!mux) {
			group_name = pinctrl_generic_get_group_name(
				pctldev, group_select);
			dev_err(pctl->dev,
				"Function %s not found for pin %d! (%s)\n",
				function_name, group->grp.pins[i], group_name);
			return -ENOENT;
		}

		pctl->pins_alt_func[group->grp.pins[i]] = (mux->is << 8) |
							  mux->os;

		if (mux->is || mux->os) {
			reg = (mux->is << GPIO_IS_SHIFT) |
			      (mux->os << GPIO_OS_SHIFT) | GPIO_PS_ALT;
		} else {
...
/* Pinctrl conf */
static int pmb887x_pconf_get(struct pinctrl_dev *pctldev, unsigned int pin,
			     unsigned long *config)
{
	struct pmb887x_pinctrl *pctl = pinctrl_dev_get_drvdata(pctldev);
...
static int pmb887x_pconf_set(struct pinctrl_dev *pctldev, unsigned int pin,
			     unsigned long *configs, unsigned int num_configs)
{
	...
	for (pinconf = 0; pinconf < num_configs; pinconf++) {
		...
		switch (param) {
		case PMB887X_PINCONF_PAD_REG:
			writel_relaxed(arg, pctl->base + GPIO_PIN(pin));
			return 0;
		...
		case PIN_CONFIG_OUTPUT_ENABLE:
			...
			set_mask |= GPIO_DIR_OUT |
				    (arg ? GPIO_DATA_HIGH : GPIO_DATA_LOW);
			break;
...
static int pmb887x_build_groups(struct pmb887x_pinctrl *pctl)
{
	...
	groups = devm_kcalloc(pctl->dev, pctl->ngroups, sizeof(*groups),
			      GFP_KERNEL);
	if (!groups)
		return -ENOMEM;
...
static int pmb887x_add_functions(struct pmb887x_pinctrl *pctl,
				 struct pinfunction *funcs)
{
	unsigned int i;
	...
	for (i = 0; i < pctl->nfuncs; i++) {
		...
		group_names = devm_kcalloc(pctl->dev, func->ngroups,
					   sizeof(*group_names), GFP_KERNEL);
		if (!group_names)
			return -ENOMEM;

		for (j = 0; j < pctl->npins; j++) {
			const struct pmb887x_desc_pin *pin_info =
				&pctl->pins[j];
			int k;

			for (k = 0; k < PMB887X_CONFIG_NUM; k++) {
				const struct pmb887x_desc_mux *pin_mux =
					&pin_info->functions[k];
				if (pin_mux->name &&
				    strcmp(pin_mux->name, func->name) == 0)
					group_names[grp_idx++] =
						pin_info->pin.name;
			}
		}
...
static void pmb887x_apply_pads(struct pmb887x_pinctrl *pctl)
{
	struct device_node *np;

	for_each_child_of_node(pctl->dev->of_node, np) {
		int npads, i;

		npads = of_property_count_elems_of_size(np, "pmb887x,pads",
							sizeof(u32));
		if (npads <= 0 || npads % 2)
			continue;
		...
		for (i = 0; i < npads; i += 2) {
			...
			if (of_property_read_u32_index(np, "pmb887x,pads", i,
						       &pin) ||
			    of_property_read_u32_index(np, "pmb887x,pads",
						       i + 1, &val))
				break;

			if (pin >= pctl->npins) {
				dev_warn(pctl->dev, "%pOFn: invalid pad %u\n",
					 np, pin);
				continue;
			}

			writel_relaxed(val, pctl->base + GPIO_PIN(pin));
		}
	}
}
...
int pmb887x_pctl_probe(struct platform_device *pdev)
{
	...
	pctl->pins_alt_func = devm_kcalloc(&pdev->dev, pctl->npins,
					   sizeof(*pctl->pins_alt_func),
					   GFP_KERNEL);
	...
	pctl->pctl_dev =
		devm_pinctrl_register(&pdev->dev, &pctl->pctl_desc, pctl);
	...
	/* Apply the board boot pad config (each *-pads child node) verbatim. */
	pmb887x_apply_pads(pctl);
